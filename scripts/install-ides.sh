...
install_codex(){
  ...
  mkdir -p "${HOME}/.local"
  /opt/node-current/bin/npm install -g --prefix "${HOME}/.local" @openai/codex >> "$LOG" 2>&1 || log "Codex install failed"
}
...
install_claude(){
  ...
  mkdir -p "${HOME}/.local"
  /opt/node-current/bin/npm install -g --prefix "${HOME}/.local" @anthropic-ai/claude-code >> "$LOG" 2>&1 || log "Claude Code install failed"
}
