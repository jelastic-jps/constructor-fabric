...
create_gui_launchers(){
  ...
  if command -v codium >/dev/null 2>&1; then
    desktop_link "VS Codium" "sh -lc 'cd ${HOME}/workspaces/constructor-fabric-workspace && exec /usr/local/bin/codium-wrap --user-data-dir=${HOME}/.config/VSCodium --goto .vscode/cfc-commands.md .'" "$(icon_for vscode)" "VS-Codium.desktop"
  fi
...
install_vscode(){
  ...
  ln -sf "$(command -v codium)" /usr/local/bin/code 2>/dev/null || true

  # Container-safe wrapper
  cat > /usr/local/bin/codium-wrap <<'WRAP'
#!/bin/sh
exec /usr/bin/codium --no-sandbox --disable-gpu --disable-dev-shm-usage "$@"
WRAP
  chmod +x /usr/local/bin/codium-wrap 2>/dev/null || true
...
install_cursor(){
  ...
  curl --noproxy '*' -fsSL "https://cursor.com/api/download?platform=linux-x64" -o /tmp/cursor.tar.gz >> "$LOG" 2>&1 \
    && tar -xzf /tmp/cursor.tar.gz -C /opt/cursor >> "$LOG" 2>&1 \
    && ln -sf /opt/cursor/cursor /usr/local/bin/cursor 2>/dev/null \
...
install_windsurf(){
  ...
  # Try apt first
  wget -qO- https://api.cursor.com/keys/windsurf.gpg 2>/dev/null | gpg --dearmor > /usr/share/keyrings/windsurf.gpg 2>/dev/null || true
  echo "deb [arch=amd64 signed-by=/usr/share/keyrings/windsurf.gpg] https://api.cursor.com/apt stable main" > /etc/apt/sources.list.d/windsurf.list 2>/dev/null || true
  apt_refresh
  timeout 300 apt-get install -y --no-install-recommends windsurf >> "$LOG" 2>&1 || {
    # Fallback to tarball
    ...
    curl --noproxy '*' -fsSL "https://api.cursor.com/download/windsurf-linux-x64" -o /tmp/windsurf.tar.gz >> "$LOG" 2>&1 \
      && tar -xzf /tmp/windsurf.tar.gz -C /opt/windsurf >> "$LOG" 2>&1 \
      && ln -sf /opt/windsurf/windsurf /usr/local/bin/windsurf 2>/dev/null \
